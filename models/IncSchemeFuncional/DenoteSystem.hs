...
lookupList key vmap =
    case lookup key vmap of
      Just v  -> v
      Nothing -> error $ "lookupList: variable no encontrada -> " ++ key
...
-- =======================================
-- Evalúa FunProp simple
-- =======================================
evalProp :: FunProp String -> Value
evalProp (Elem _ _)        = V1
...
convertSingleProp vmap i prop =
    ...
    in (show i ++ "_" ++ toVar, toVertex, evalProp prop)
    ...
    Just fprop -> 
      case fprop of
        SetProp props ->
          ...
          in Apply applies
        otherProp ->
          let toVar = getElemVar otherProp
              toVertex = lookupList toVar vmap
          in Apply [(name, toVertex, evalProp otherProp)]
    Nothing -> Apply [(name, fromVertex, V2)] -- indefinido
